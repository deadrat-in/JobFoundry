...
async def process_unscored_jobs(
    ...
    """
    1. Fetches unscored jobs from the store.
    2. Scores each job against the master resume.
    3. If the job passes threshold:
       - Calls the tailoring bridge to tailor the resume and render PDF / ATS text.
       - Saves the artifacts to disk.
       - Updates job status to 'tailored' with tailored_resume_id.
    4. If the job fails threshold:
       - Marks status as 'rejected_by_score'.
    """
                ...
                if tailor_bridge and tailor_bridge.base_url:
                    try:
                        ...
                        if tailor_res and tailor_res.status == "completed":
                            ...
                            tailored_resume_id = tailor_res.task_id or f"tailored-{job_id}-{uuid.uuid4().hex[:8]}"
