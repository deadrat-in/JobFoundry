...
def test_load_config_defaults(monkeypatch):
    """Defaults include the internal loopback Tailor endpoint."""
    ...
    monkeypatch.delenv("TAILOR_PORT", raising=False)
    ...
    assert config.resume_ops_url == "http://127.0.0.1:8081"
...
def test_load_config_custom_env(monkeypatch):
    """Supported environment overrides are reflected in scorer config."""
...
def test_load_config_tailor_port(monkeypatch):
    """TAILOR_PORT selects the internal loopback Tailor endpoint."""
...
def test_legacy_resume_ops_url_cannot_select_remote_endpoint(monkeypatch):
    """The removed legacy variable cannot redirect Tailor traffic remotely."""

    monkeypatch.delenv("TAILOR_PORT", raising=False)
    monkeypatch.setenv("RESUME_OPS_URL", "https://tailor.example.com")
    config = load_config()
    assert config.resume_ops_url == "http://127.0.0.1:8081"
