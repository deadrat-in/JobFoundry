...
try {
  ...
  // fetch() — keyword fallback to config/profile.yml's target_roles (browser
  // port). Upstream wrote a real profile.yml into an isolated tmp cwd and
  // expected vdab to fall back to its target_roles. The browser port has no
  // profile.yml fallback (see the _profile-keywords port), so an entry with
  // empty vdab.keywords[] must throw — the same behaviour upstream exhibits
  // when no profile exists. The assertion is therefore the throw case.
  {
    // No entry keywords AND no profile fallback (browser port) → throws.
    let threwNoKeywords = false;
    try {
      await vdab.fetch({ name: 'VDAB empty', vdab: {} }, mkCtx({}));
    } catch { threwNoKeywords = true; }
    if (threwNoKeywords) pass('vdab.fetch() throws when vdab.keywords[] is empty (browser port has no profile.yml fallback)');
