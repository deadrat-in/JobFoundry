...
try {
  ...
  // Browser port: inlined pure URL normalization helpers
  function promoteKnownFragmentIdentity(u) {
    if (u.hostname.toLowerCase() !== 'app.mokahr.com') return;
    const match = /^#\/job\/([^/?#]+)(?:\?[^#]*)?$/.exec(u.hash);
    if (!match) return;
    let jobId;
    try { jobId = decodeURIComponent(match[1]); } catch { return; }
    if (jobId) u.searchParams.set('mokahr_job_id', jobId);
  }
